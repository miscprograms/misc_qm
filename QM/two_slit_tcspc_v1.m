...
%Generate photons as per sinusoidal wave
%% signal takes values in [1:25] range with mean=13 photons.
dc_offset=13;
A=12;
fs=f*N0;
signal=dc_offset + A.*sin(2*pi*f.*[0:1:N0-1]/fs); 
signal=round(signal); %Integer number of photons . rounded to integers.
...
for loopCount=1:100*10000, %*10, %*50, %280000, %100*10000, %numLoop,
   ...
   for sampleCount=1:N0,
    ...
    % In QM 2-slit experiment, each carrier frequency cycle, only one particle or set of particle arrives, 
    %at one of 50 time instants/bins.
    %In classical 2-slit experiment, we will use all time instants sequentially.
    ...
    for sample=sampleNum, %N0,
        ...
        %K1=5*2;  
        K1=signal(sampleNum); %Index the number of photons from signal array.
